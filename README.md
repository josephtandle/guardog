...
VirusTotal stays available as an optional second opinion for a URL or file hash. OSV, reputation, and pattern checks continue working without it.
...
| Verdict | Meaning |
|---------|---------|
| SILENT | No blocking evidence found |
| WHINE | Suspicious, review before relying on it |
| BARK | Dangerous, guarded install is blocked |
...
3. Risky package metadata and code patterns
4. Optional VirusTotal results when a key and target are provided
...
Guardog has no Telegram or external notification integration. Runtime history and secrets stay in the local Guardog state folder. Do not include keys, tokens, `.env` files, or unredacted scan logs in bug reports.
