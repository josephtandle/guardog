...
const CACHE_PATH = join(DATA_DIR, 'threat-intel-cache.json');
...
export class ThreatIntel {
  ...
  /**
   * Strip semver range operators to get a queryable version string.
   * Returns null if the version is a range, wildcard, or non-semver reference.
   * @param {string} versionStr - Raw version from package.json
   * @returns {string|null}
   */
  cleanVersion(versionStr) {
    if (!versionStr || versionStr === '*' || versionStr === 'latest') return null;
    // Non-registry references: workspace, file, git, github
    if (/^(workspace:|file:|git|github:|bitbucket:|npm:)/.test(versionStr)) return null;
    // Strip leading operators (^, ~, >=, >, <=, <, =, spaces)
    const stripped = versionStr.replace(/^[\^~>=<\s]+/, '').split(/\s+/)[0].split('||')[0].trim();
    // Must look like X, X.Y, or X.Y.Z (with optional pre-release/build)
    if (/^\d+(\.\d+)*(-[\w.]+)?(\+[\w.]+)?$/.test(stripped)) return stripped;
    return null;
  }

  /**
   * Query OSV in batch (up to 1000 queries at once)
   * Includes version when available so OSV only returns vulns affecting that version.
   * @param {Array<Object>} packages - Array of {name, ecosystem, version?}
   * @returns {Promise<Array>} Normalized findings
   */
  async queryOSVBatch(packages) {
    ...
    for (let i = 0; i < packages.length; i += batchSize) {
      ...
      const queries = batch.map(pkg => {
        ...
        // Including version makes OSV return only vulns that affect this specific
        // version, eliminating false positives from old fixed advisories.
        const v = pkg.version ? this.cleanVersion(pkg.version) : null;
        if (v) q.version = v;
        return q;
      });
  ...
  normalizeOSV(vuln, packageName, ecosystem) {
    // OSV returns summary, details (long markdown), or aliases — prefer summary,
    // fall back to first line of details, then aliases list.
    let summary = vuln.summary;
    if (!summary && vuln.details) {
      // First non-empty line of the markdown details
      summary = vuln.details.split('\n').find(l => l.trim().length > 10)?.trim()?.substring(0, 200);
    }
    if (!summary && vuln.aliases?.length) {
      summary = `See ${vuln.aliases.join(', ')}`;
    }
    summary = summary || 'No summary available';

    // Extract affected version range for context
    const ranges = vuln.affected?.[0]?.ranges || [];
    const fixedAt = ranges.flatMap(r => r.events || []).find(e => e.fixed)?.fixed;
   ...
   * @returns {Object|null} Normalized finding or null — returns null if ecosystem cannot be determined
   */
  normalizeNVD(item) {
    const cve = item.cve;
    if (!cve) return null;
    ...
    // Require explicit npm/pypi signals before matching against installed packages.
    // NVD covers OS, firmware, enterprise software, hardware — most entries are NOT
    // npm packages. Without a confirmed ecosystem we would match e.g. "Open Cluster
    // Management" → npm `open`, producing false positives like CVE-2026-4740.
    let ecosystem = 'unknown';
    if (/\b(npm|node\.?js|javascript|js library|js module)\b/i.test(description)) {
      ecosystem = 'npm';
    } else if (/\b(pip|pypi|python package)\b/i.test(description)) {
      ecosystem = 'pypi';
    }

    // Drop entries we cannot confidently map to an npm/pypi package
    if (ecosystem === 'unknown') return null;

    // Extract package name only after confirming ecosystem
    let pkg = 'unknown';
    const npmMatch = description.match(/(?:npm package |node module |package )["']?(\S+?)["']?(?:\s|,|\.)/i);
    if (npmMatch) pkg = npmMatch[1].replace(/['"]/g, '');

    if (pkg === 'unknown') {
      // Fallback: first word after "in " that looks like a package name
      const fallback = description.match(/\bin ([\w@/-]+)/i);
      if (fallback) pkg = fallback[1];
    }
    ...
    try {
      ...
      return vulnerabilities
        ...
        .map(v => ({
          ...
          // CISA KEV covers OS, firmware, enterprise software — not npm packages.
          // Tag as 'infrastructure' so crossReference skips them for npm matching.
          // They are reported separately in the threat intel summary.
   ...
   * @returns {Object} { matches: Array, infrastructureThreats: Array, summary: Object }
   */
  crossReference(installedPackages, findings) {
    const packageSet = new Set(installedPackages.map(p => p.name.toLowerCase()));

    // Separate infrastructure threats (CISA KEV, unknown ecosystem) from npm/pypi findings
    const infrastructureThreats = findings.filter(f =>
      f.ecosystem === 'infrastructure' || f.ecosystem === 'unknown'
    );

    // Only match findings with a confirmed npm/pypi ecosystem against installed packages.
    // This prevents OS/firmware/enterprise CVEs from matching npm package names by accident.
    const matches = findings.filter(f =>
      (f.ecosystem === 'npm' || f.ecosystem === 'pypi') &&
    ...
    const summary = {
      ...
      infrastructureThreats: infrastructureThreats.length,
      severity: {
    ...
    return { matches, infrastructureThreats, summary };
  }
